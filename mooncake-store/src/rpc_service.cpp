...
tl::expected<void, ErrorCode> WrappedMasterService::GracefulUnmountSegment(
    ...
    return execute_rpc(
        ...
        [&] {
            return master_service_.GracefulUnmountSegment(
                segment_id, client_id, grace_period_ms);
        },
        [&](auto& timer) {
            timer.LogRequest("segment_id=", segment_id, ", client_id=",
                             client_id, ", grace_period_ms=", grace_period_ms);
        },
        [] {
            MasterMetricManager::instance()
