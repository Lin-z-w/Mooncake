...
TEST_F(MasterServiceSSDSnapshotTest, PutEndBothReplica) {
    ...
    EXPECT_TRUE(
        service_->PutEnd(client_id, key, "default", ReplicaType::DISK).has_value());
...
TEST_F(MasterServiceSSDSnapshotTest, PutRevokeDiskReplica) {
    ...
    EXPECT_TRUE(
        service_->PutRevoke(client_id, key, "default", ReplicaType::DISK).has_value());
...
TEST_F(MasterServiceSSDSnapshotTest, PutRevokeMemoryReplica) {
    ...
    ASSERT_TRUE(
        service_->PutStart(client_id, key, "default", slice_length, config).has_value());
    EXPECT_TRUE(
        service_->PutRevoke(client_id, key, "default", ReplicaType::MEMORY).has_value());
...
TEST_F(MasterServiceSSDSnapshotTest, PutStartExpires) {
    ...
    auto test_discard_replica = [&](ReplicaType discard_type) {
        ...
        auto put_end_result = service_->PutEnd(client_id, key, "default", reserve_type);
        ...
        put_end_result = service_->PutEnd(client_id, key, "default", discard_type);
        EXPECT_TRUE(put_end_result.has_value());
