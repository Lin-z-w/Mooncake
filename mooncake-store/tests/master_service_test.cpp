...
TEST_F(MasterServiceTest, GracefulUnmountSegment_SetsCorrectStatus) {
    ...
    auto graceful_result =
        service_->GracefulUnmountSegment(segment.id, client_id, /*grace_period_ms=*/1000);
    ASSERT_TRUE(graceful_result.has_value())
        << "Graceful unmount should succeed: " << toString(graceful_result.error());
...
TEST_F(MasterServiceTest, GracefulUnmountSegment_Idempotent) {
    ...
    auto result1 = service_->GracefulUnmountSegment(
        segment.id, client_id, /*grace_period_ms=*/1000);
    ASSERT_TRUE(result1.has_value());

    // Second graceful unmount on the same segment should also succeed (idempotent)
    auto result2 = service_->GracefulUnmountSegment(
        segment.id, client_id, /*grace_period_ms=*/1000);
    EXPECT_TRUE(result2.has_value())
        << "Graceful unmount should be idempotent";
...
TEST_F(MasterServiceTest, GracefulUnmountSegment_TimerExpiresAndUnmounts) {
    ...
    // After timer expires, segment should be fully unmounted (UNDEFINED or error)
    auto status_after = service_->QuerySegmentStatus(segment.name);
