...
TEST_F(ObjectDataTypeTest, PutStartWithDataType) {
    ...
    auto result = service->PutStart(put_client, "key_weight", "default", 1024, config);
    ASSERT_TRUE(result.has_value());
    EXPECT_FALSE(result.value().empty());

    auto end_result =
        service->PutEnd(put_client, "key_weight", "default", ReplicaType::MEMORY);
...
TEST_F(ObjectDataTypeTest, PutStartDefaultDataType) {
    ...
    auto result = service->PutStart(put_client, "key_default", "default", 1024, config);
    ASSERT_TRUE(result.has_value());
    EXPECT_FALSE(result.value().empty());
}
