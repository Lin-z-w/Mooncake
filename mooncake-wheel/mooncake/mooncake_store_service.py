...
class MooncakeStoreService:
    ...
    async def start_http_service(self, port: int = 8080):
        ...
        app.add_routes([
            ...
            web.post('/api/allocate_mount', _timed_handler("ALLOCATE_MOUNT", self.handle_allocate_mount)),
            web.post('/api/unmount_free', _timed_handler("UNMOUNT_FREE", self.handle_unmount_free)),
    ...
    async def handle_allocate_mount(self, request):
        try:
            data = await request.json()
            size = data.get("size")
            protocol = data.get("protocol", self.config.protocol)
            ...
            logging.error("ALLOCATE_MOUNT error: %s", e)
            return web.Response(
                status=500,
                text=json.dumps({"error": str(e)}),
                content_type="application/json"
    ...
    async def handle_unmount_free(self, request):
        try:
            data = await request.json()
            segment_ids = data.get("segment_ids", [])
            if isinstance(segment_ids, str):
            ...
            logging.error("UNMOUNT_FREE error: %s", e)
            return web.Response(
                status=500,
                text=json.dumps({"error": str(e)}),
                content_type="application/json"
